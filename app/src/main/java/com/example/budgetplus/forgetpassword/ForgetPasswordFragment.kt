...
class ForgetPasswordFragment : BaseFragment(), View.OnClickListener, IOnBackPressed {
    ...
    private val forgetPasswordViewModel: Lazy<ForgetPasswordViewModel> =
        KoinJavaComponent.inject(ForgetPasswordViewModel::class.java)
    ...
    private fun initObservers() {
        forgetPasswordViewModel.value.resetPassword
            .observe(viewLifecycleOwner, _resetPasswordObserver)
        forgetPasswordViewModel.value.controlResetCode
            .observe(viewLifecycleOwner, _controlResetCodeObserver)
        forgetPasswordViewModel.value.changePassword
            .observe(viewLifecycleOwner, _changePasswordObserver)
    ...
    private fun resetPasswordService() {
        if (checkValidation()) {
            forgetPasswordViewModel.value.resetPassword(binding.ETForgetEmail.text.toString())
    ...
    private fun controlResetCodeService() {
        ...
        forgetPasswordViewModel.value.controlResetCode(emailStr, codeStr)
    }

    private fun changePasswordService() {
        if (checkValidationPassword()) {
            forgetPasswordViewModel.value.changePassword(getChangePasswordRequestBodyModel())
        }
    }
    ...
    //Observers
    private val _resetPasswordObserver = Observer<Resource<ResponseBody>> {
        when (it.status) {
            Status.ERROR -> {
                hide()
    ...
    private val _controlResetCodeObserver = Observer<Resource<ResponseBody>> {
        when (it.status) {
            Status.SUCCESS -> {
                hide()
    ...
    private val _changePasswordObserver = Observer<Resource<ResponseBody>> {
        when (it.status) {
            Status.LOADING -> {
                show()
            }
